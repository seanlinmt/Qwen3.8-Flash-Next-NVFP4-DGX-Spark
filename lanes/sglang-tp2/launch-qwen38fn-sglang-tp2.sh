...
HEAD_IP="${HEAD_IP:-192.168.192.1}"
INIT_PORT="${INIT_PORT:-29511}"
PORT="${PORT:-8000}"
...
if [ -n "${ADDR_RANGE:-}" ]; then
  EXTRA_NCCL_ARGS+=("-e" "NCCL_IB_ADDR_RANGE=$ADDR_RANGE")
elif [ -n "${NCCL_IB_ADDR_RANGE:-}" ]; then
  EXTRA_NCCL_ARGS+=("-e" "NCCL_IB_ADDR_RANGE=$NCCL_IB_ADDR_RANGE")
fi
...
docker run --gpus all -d \
  ...
  -e NCCL_MAX_NCHANNELS="${NCCL_MAX_NCHANNELS:-4}" -e NCCL_MIN_NCHANNELS="${NCCL_MIN_NCHANNELS:-4}" \
