...
LANE="${LANE:-AI}"
case "$LANE" in
  AI) HEAD_IP="192.168.177.11"; WORKER_IP="192.168.177.12"; MPORT="${MPORT:-29532}"; ADDR_RANGE="192.168.176.0/22" ;;  # AI1 head, AI2 worker (177.x + 178.x)
  B)  HEAD_IP="192.168.192.2";  WORKER_IP="192.168.192.4";  MPORT="${MPORT:-29531}"; ADDR_RANGE="192.168.192.0/22" ;;  # Reddie head, Spark4 worker
  A)  HEAD_IP="192.168.192.1";  WORKER_IP="192.168.192.3";  MPORT="${MPORT:-29532}"; ADDR_RANGE="192.168.192.0/22" ;;  # Bluey head, Asusi worker
...
docker run --gpus all -d --name "$NAME" --restart no \
  ...
  -e NCCL_IB_ADDR_RANGE="${NCCL_IB_ADDR_RANGE:-$ADDR_RANGE}" \
